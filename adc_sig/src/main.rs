...
fn main() -> ! {
    ...
    // 为特定通道设置ADC采样时间
    adc1.set_channel_sample_time(0, SampleTime::T_1);
    ...
    loop {
        ...
        let voltage = ad_value as f32 / 4095f32 * 3.3;
        // 将ADC读数转换为电压
        ...
        println!(
            ...
            ad_value, voltage, temperature, vref
        );
        oled.show_num(1, 9, ad_value as u32, 4);
        // 获取电压整数部分
        oled.show_num(2, 9, voltage as u32, 1);
        // 获取电压小数部分
        oled.show_num(2, 11, ((voltage * 100.0) % 100.0) as u32, 2);
