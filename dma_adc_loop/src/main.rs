...
fn main() -> ! {
    ...
    // DMA 环形缓冲区的前半区（buf[0]）
    let first_half = &buf[0];
    // DMA 环形缓冲区的后半区（buf[1]）
    let second_half = &buf[1];

    // 每个半区有 8 个数：AD0 AD1 AD2 AD3 AD0 AD1 AD2 AD3...

    // 显示第一半区的 AD0~AD3（第一组转换结果）
    oled.show_num(1, 5, first_half[0].into(), 4); // AD0
    oled.show_num(2, 5, first_half[1].into(), 4); // AD1
    oled.show_num(3, 5, first_half[2].into(), 4); // AD2
    oled.show_num(4, 5, first_half[3].into(), 4); // AD3

    // 显示第二半区的 AD0~AD3（第一组转换结果）
    oled.show_num(1, 8, second_half[0].into(), 4); // AD0
    oled.show_num(2, 8, second_half[1].into(), 4); // AD1
    oled.show_num(3, 8, second_half[2].into(), 4); // AD2
    oled.show_num(4, 8, second_half[3].into(), 4); // AD3
