//! OLED 通用工具函数封装
use super::fonts::OLED_FONT;  //父模块

//use embedded_hal::digital::v2::OutputPin;
// 为 OLED 的 I2C 通信提供引脚操作标准
use embedded_hal::digital::v2::OutputPin;
// 开漏输出、输出模式
use stm32f1xx_hal::gpio::{OpenDrain, Output, PB8, PB9};

// 定义结构体，使用泛型让OLED结构体适配不同的引脚
pub struct OLED<Scl, Sda>
// 添加约束：必须实现OutputPin这个trait
where
    Scl: OutputPin,
    Sda: OutputPin,
{
    ...
    Sda: OutputPin,
{
    ...
    pub fn new(scl: Scl, sda: Sda) -> Self {
        // 传入已配置完成的scl和sda引脚，创建OLED结构体实例
        let mut oled = OLED { scl, sda };
        oled.init();
        // 返回oled实例，已完成引脚配置和初始化
        oled
    }
