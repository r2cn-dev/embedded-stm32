...
fn main() -> ! {
    ...
    // 初始化 OLED 显示屏
    println!("load oled...");
    let mut oled = oled::simple
    ::init_oled(gpiob.pb8, gpiob.pb9, &mut gpiob.crh);
    ...
    let mut qei = Timer
    ::new(tim3, &clocks).qei((pa6, pa7), &mut afio.mapr, QeiOptions::default());

    //let qei_options = QeiOptions {
    //slave_mode: SlaveMode::EncoderMode1, // 1倍频模式（二选一即可）
    //slave_mode: SlaveMode::EncoderMode2, // 也是1倍频，效果完全一致
    //auto_reload_value: u16::MAX, // 保持默认计数范围（0~65535，溢出回绕）
    //};

    //let mut qei = Timer::new(tim3, &clocks)
    //.qei((pa6, pa7), &mut afio.mapr, qei_options);

    oled.show_string(1, 1, "Cnt:");
    println!("loop ...");
    loop {
        // 获取当前编码器计数
        let tim3_cnt = get_tim3_cnt(&mut qei)/4;
...
where
//设计复用：STM32 的定时器通道是多功能共享的,同一个定时器通道（如 TIM3_CH1）既可以用于 QEI，也可以用于 PWM 输入、普通输入捕获
//复用了pwm_input::Pins这个 trait，让它同时承担所有定时器通道复用场景的引脚校验。
